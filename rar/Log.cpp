...
{	
	...
	OutputDebugString("CLog construction\n");
...
void CLog::CreateFileName()
{
	...
	GetLocalTime(&st);
	sprintf(sztime, "%04d年%02d月%02d%日02d时%02d分%02d秒%.log", 
		st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
	lstrcpy(m_szFileName, sztime);
}
...
BOOL CLog::OpenConsole(BOOL bOpen)
{
	if(bOpen)
	{
		if(!m_bConsoleOpened)
		{
			if(AllocConsole())
			{
				SetConsoleTitle(_T("AutoBackup output"));
				/*COORD coord;
				coord.X = 120;
				coord.Y = 60;
...
WORD CLog::FormatBuf(const char *oldBuf, char *newBuf, WORD max_new_buff_len)
{
	try
	{
		...
		*/
		sprintf(sztime, "HB %04d-%02d-%02d-w%d %02d:%02d:%02d.%03d ", 
...
LPCTSTR CLog::GetAppRunTime()
{
	...
	sprintf(szTime, "%dday%02d:%02d:%02ds.%3.6fms", day, hour, min, sec, ms);
	return szTime;
}

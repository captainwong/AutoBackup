...
public:
	...
	static BOOL DeleteFolderAndAllSubFolder_Files(LPCTSTR folderPath){
		...
		CString cmd = _T("");
		cmd.Format(_T("rd /s /q \"%s\""), folderPath);
		BOOL ret = system(cmd) == 0;
		cmd.Format("echo delete %s\n", ret ? "success" : "failed");
		system(cmd);
		return ret;
	}
	...
	static BOOL RenameFile03d(LPCTSTR lpszFilePath){
		...
		int nums = atoi(num.GetBuffer(num.GetLength()));
		num.ReleaseBuffer();
...
protected:
	static BOOL FindFirstFileExists(LPCTSTR lspzPath, DWORD dwFilter){
		...
		if(lstrlen(lspzPath) > MAX_PATH)
			GetShortPathName(lspzPath, realPath, MAX_PATH);
		else
			lstrcpy(realPath, lspzPath);
